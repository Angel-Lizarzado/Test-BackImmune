...
describe('FilmRepo', () => {
  ...
  describe('query', () => {
    it('should return films with pagination', async () => {
      ...
      FilmModel.find.mockReturnValue({
        ...
        exec: jest.fn().mockResolvedValue(mockFilms), // Retorna los filmes simulados
      });

      const result = await filmRepo.query(1, 6, 'Action');
      expect(result).toEqual(mockFilms);
      expect(FilmModel.find).toHaveBeenCalledWith({ genre: 'Action' });
      expect(FilmModel.find().skip).toHaveBeenCalledWith(0); // (1-1) * 6
  ...
  describe('count', () => {
    it('should return the count of films with genre filter', async () => {
      FilmModel.countDocuments.mockReturnValue({
        exec: jest.fn().mockResolvedValue(5), // Simula que hay 5 filmes
      });
    ...
    it('should return the count of all films without genre filter', async () => {
      FilmModel.countDocuments.mockReturnValue({
        exec: jest.fn().mockResolvedValue(10), // Simula que hay 10 filmes
      });
  ...
  describe('queryById', () => {
    it('should return a film by id', async () => {
      ...
      FilmModel.findById.mockReturnValue({
        populate: jest.fn().mockReturnThis(),
        exec: jest.fn().mockResolvedValue(mockFilm), // Retorna el filme simulado
      });
    ...
    it('should throw HttpError if film not found', async () => {
      FilmModel.findById.mockReturnValue({
        populate: jest.fn().mockReturnThis(),
        exec: jest.fn().mockResolvedValue(null), // Simula que no se encontró el filme
      });
  ...
  describe('create', () => {
    it('should create a new film', async () => {
      ...
      FilmModel.create.mockResolvedValue(mockFilm); // Retorna el nuevo filme simulado
  ...
  describe('delete', () => {
    it('should delete a film by id', async () => {
      FilmModel.findByIdAndDelete.mockReturnValue({
        exec: jest.fn().mockResolvedValue(true), // Simula que el filme fue eliminado
      });
